package ru.devsp.app.mtgcollections.repository;

import android.arch.lifecycle.LiveData;

import java.util.List;

import javax.inject.Inject;
import javax.inject.Singleton;

import ru.devsp.app.mtgcollections.model.db.LibraryDao;
import ru.devsp.app.mtgcollections.model.objects.Library;
import ru.devsp.app.mtgcollections.model.objects.LibraryInfo;
import ru.devsp.app.mtgcollections.tools.AppExecutors;

/**
 * Обработка колод
 * Created by gen on 03.10.2017.
 */
@Singleton
public class LibrariesRepository {

    private final AppExecutors mAppExecutors;

    private LibraryDao mDao;

    @Inject
    public LibrariesRepository(AppExecutors appExecutors, LibraryDao dao) {
        mAppExecutors = appExecutors;
        mDao = dao;
    }

    public void save(Library item) {
        mAppExecutors.diskIO().execute(() -> mDao.insert(item));
    }

    public void update(Library item) {
        mAppExecutors.diskIO().execute(() -> mDao.update(item));
    }


    public LiveData<List<Library>> getAllLibraries() {
        return mDao.getLibrariesList();
    }

    public LiveData<List<LibraryInfo>> getLibrariesInfoList() {
        return mDao.getLibrariesInfoList();
    }

}
